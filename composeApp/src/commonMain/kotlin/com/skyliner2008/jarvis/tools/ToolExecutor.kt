...
import com.skyliner2008.jarvis.tools.trading.TradingToolExecutor
import com.skyliner2008.jarvis.tools.camera.CameraToolExecutor
import com.skyliner2008.jarvis.tools.system.SystemToolExecutor
...
object ToolExecutor {
    ...
    fun init(client: HttpClient, geminiService: com.skyliner2008.jarvis.data.GeminiService) {
        ...
        return try {
            ...
            val result = when {
                ...
                ToolRegistry.isStrategyTool(routedToolName) -> {
                    com.skyliner2008.jarvis.tools.strategy.StrategyToolExecutor.execute(routedToolName, routedArgs)
                }
                // ─── Built-in Tools ────────────────────────────────────────
    ...
    private suspend fun executeLiveControl(toolName: String, args: Map<String, String>): String {
        ...
        return when (toolName) {
            ...
            "voice_get_profiles" ->
                "📋 รายชื่อเสียงที่ใช้ได้:\n${com.skyliner2008.jarvis.data.GeminiVoiceProfiles.getVoiceListSummary()}"
            "voice_set_profile" -> {
        ...
        return when (action) {
            "create" -> {
                ...
                if (!factorsArg.isNullOrBlank()) {
                    com.skyliner2008.jarvis.automation.AnticipationConfigManager.setFactors(symbolBase, factorsArg.split(",").map { it.trim() })
                }
                val addFactorsArg = args["add_factors"]?.trim()
                if (!addFactorsArg.isNullOrBlank()) {
                    addFactorsArg.split(",").map { it.trim() }.forEach {
                        com.skyliner2008.jarvis.automation.AnticipationConfigManager.addFactor(symbolBase, it)
                    }
                }
                val removeFactorsArg = args["remove_factors"]?.trim()
                if (!removeFactorsArg.isNullOrBlank()) {
                    removeFactorsArg.split(",").map { it.trim() }.forEach {
                        com.skyliner2008.jarvis.automation.AnticipationConfigManager.removeFactor(symbolBase, it)
                    }
                }
            ...
            "config" -> {
                val factorsArg = args["factors"]?.trim()
                if (!factorsArg.isNullOrBlank()) {
                    com.skyliner2008.jarvis.automation.AnticipationConfigManager.setFactors(symbol, factorsArg.split(",").map { it.trim() })
                }
                val addFactorsArg = args["add_factors"]?.trim()
                if (!addFactorsArg.isNullOrBlank()) {
                    addFactorsArg.split(",").map { it.trim() }.forEach {
                        com.skyliner2008.jarvis.automation.AnticipationConfigManager.addFactor(symbol, it)
                    }
                }
                val removeFactorsArg = args["remove_factors"]?.trim()
                if (!removeFactorsArg.isNullOrBlank()) {
                    removeFactorsArg.split(",").map { it.trim() }.forEach {
                        com.skyliner2008.jarvis.automation.AnticipationConfigManager.removeFactor(symbol, it)
                    }
                }
            ...
            "recommend" -> {
                val rec = com.skyliner2008.jarvis.automation.AnticipationConfigManager.getRecommendedFactors(symbol)
                buildString {
