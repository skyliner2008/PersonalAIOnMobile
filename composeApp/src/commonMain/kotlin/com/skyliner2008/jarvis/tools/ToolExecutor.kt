...
object ToolExecutor {
    ...
    /**
     * ระบบปลุก AI (คาดการณ์ล่วงหน้า) — แยกจากระบบแจ้งเตือน Signal
     * รายละเอียดทั้งหมดอยู่ใน [com.skyliner2008.jarvis.automation.wake.AnticipationToolActions]
     */
    private suspend fun executeSignalAnticipation(args: Map<String, String>): String {
        val trader = _tradingExecutor
        val scanner: (suspend (String) -> Map<String, String>)? =
            if (trader != null) { raw -> trader.anticipationScan(raw) } else null
        return com.skyliner2008.jarvis.automation.wake.AnticipationToolActions(
            createAlert = { executeManageAlerts(it) },
            scanner = scanner
        ).execute(args)
    }
