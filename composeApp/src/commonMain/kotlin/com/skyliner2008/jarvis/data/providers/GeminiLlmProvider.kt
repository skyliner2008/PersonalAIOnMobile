package com.skyliner2008.jarvis.data.providers
...
class GeminiLlmProvider(
    ...
    override suspend fun listModels(apiKey: String): List<LlmModelInfo> {
        ...
        return try {
            ...
            val modelList = json.decodeFromString<com.skyliner2008.jarvis.data.ModelListResponse>(respText)
            // ซิงก์ registry กลางแบบ dynamic ตามโมเดลจริงที่มีใน API ทันที
            com.skyliner2008.jarvis.data.ModelConfig.updateAvailableModels(modelList.models)

            val mappedList = modelList.models.filter {
                val cleanId = it.name.removePrefix("models/")
                !com.skyliner2008.jarvis.data.ModelConfig.isModelDead(cleanId) &&
                ...
                LlmModelInfo(
                    ...
                    supportsFunctions = com.skyliner2008.jarvis.data.ModelConfig.supportsNativeTools(model.name),
