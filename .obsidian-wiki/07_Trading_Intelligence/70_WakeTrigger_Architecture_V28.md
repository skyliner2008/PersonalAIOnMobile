...
## 5.3 ปัจจัย × TF — ประเมินทุก TF ทุกนาที (P16, 2026-09-20)

**ปัญหาเดิม**: ~90 ปัจจัยดูเฉพาะ TF ของ job (M15), ปลุกได้ครั้งเดียวต่อแท่ง M15 (≈4/ชม., ~144/วัน)
เหตุการณ์ M1/M5 กลางแท่งต้องรอแท่ง M15 ปิด และการเรียนรู้ไม่รู้ว่าปัจจัยไหนเหมาะกับ TF ไหน

**ตอนนี้**
- `WakeContext.forTf(tf)` = มุมมองเดียวกันแต่ `primary` เป็น TF นั้น (แชร์ `SeriesSet` — ไม่คำนวณอินดิเคเตอร์ซ้ำ)
- `WakeTriggerRegistry.scanAllTf` ประเมินปัจจัยที่ใช้ได้ทุก TF บน **M1/M5/M15/H1/H4** (+TF ของ job) ทุกนาที;
  ปัจจัย `WakeTfProfile.FIXED` (26 ตัว: ภาพ 5TF, intermarket, ข่าว, session, memory) ประเมินครั้งเดียว
- ทุก TF ดึง 800 แท่ง (FULL_SET) — เดิม M1 300 / M5 500 / H4 400
- **เรียนรู้ทุกปัจจัย × ทุก TF** (ฟรี) — `AnticipationFactorOutcome.factor_tf` (migration 16 สร้างตารางใหม่เพราะ
  UNIQUE เปลี่ยนเป็น `(signal_id, factor_id, factor_tf)`), วัดผล 12 แท่ง **ของ TF นั้น** หน่วย ATR ของ TF นั้น
- **ปลุก AI ได้เฉพาะปัจจัย × TF ที่เหมาะ**: `WakeTfProfile.defaultWakeTfs` (M1 = แตะ/กวาดระดับ, climax, anomaly;
  M5 = ยืนยันโครงสร้าง/divergence; M15 = ทุกตัว; H1/H4 = โครงสร้าง/เทรนด์/รูปแบบ) + TF ของ job เสมอ
  → ปรับด้วยผลจริง `WakeLearningStore.tfVerdict`: ⬇️ ลดชั้น (n≥40, avg R ≤ 0) / ⬆️ เลื่อนชั้น TF นอกชุด (n≥40, avg R ≥ 0.2, ไปตามทิศ ≥ 55%)
- Governor: cooldown ต่อ **ปัจจัย@TF@ทิศ** นับเป็นแท่งของ TF นั้น; ระยะห่างขั้นต่ำ 1 นาที (แทน "ครั้งเดียวต่อแท่ง");
  เหตุการณ์ที่ติดระยะห่าง/งบถูกพกไปการปลุกถัดไป ≤ 10 นาที (บรรทัด "(เกิดเมื่อ N นาทีก่อน)")
- งบเริ่มต้น 40/ชม., 3,000/วัน (ผู้ใช้มี API key 7 ตัว สลับอัตโนมัติ); ชั้น alert ไม่ throttle 90 วิ กับงานปลุก AI แล้ว
- รหัสการปลุก `SYMBOL|TF|นาทีที่สแกน`; payload `wake_triggers` = `ID@tf`; เหตุการณ์ใน prompt เรียง H4 → M1

**จำลองจากแท่ง BTC จริง 24 ชม. (สแกนทุกนาที, `wake_factor_replay.py --step 1m`)**
- เหตุการณ์ที่ปลุกได้/ชม.: M1 8.6 · M5 20 · M15 16.5 · H1 1.8 · H4 0.3 (ทั้งหมดที่บันทึกเพื่อเรียนรู้: M1 219 · M5 49)
- ปลุก ~14 ครั้ง/ชม. (~345/วัน คริปโต 24/7), เฉลี่ย 2.6 เหตุการณ์ต่อการปลุก
- ประเมิน 115 ปัจจัย × 5 TF ใช้ ~10–13 ms/รอบบน PC
- ตัด M1 ออกจาก oscillator/impulse/volume spike/delta (ยิง 70–210 ครั้ง/วันบน M1) — ยังเรียนรู้อยู่ เลื่อนชั้นได้ถ้าผลดี

## 5.4 ขนาดข้อมูลหลังประเมินทุก TF ทุกนาที (P17, 2026-09-20)

วัดจากมือถือจริงหลัง P16: **~6,000 แถว/วัน/สินทรัพย์** (M1 73%) — เก็บดิบ 365 วัน = ~2 ล้านแถว (หลายร้อย MB)

- ตาราง `AnticipationFactorStat` (migration 17): ผลรวมต่อ (ปัจจัย × TF × ทิศ × บริบท) เก็บ **ถาวร**
- `WakeLearningStore.foldOldOutcomes` (งานรายวัน): ยุบแถวที่วัดผลแล้ว **และไม่ได้ปลุก AI** ที่เก่ากว่า `FOLD_DAYS` (3 วัน)
  เข้าสถิติสะสม แล้วลบแถวดิบ; `foldAgedBeforeDelete` ยุบทุกแถวที่วัดผลแล้วก่อนลบตาม retention (ใน transaction เดียวกับการลบ)
- `load()` รวมสถิติสะสม + แถวดิบ → การเรียนรู้ไม่ "ลืม" เมื่อ prune (ตรวจบนสำเนาฐานข้อมูลจริง: ยุบ 2 รอบแล้วค่าสถิติตรงเป๊ะ)
- `setFactorOutcomeAiDecision` เขียนเฉพาะแถวที่ปลุกจริง (`woke = 1`) — เดิมเขียนทุกแถวของการสแกน ทำให้แถว
  "เก็บสถิติอย่างเดียว" ยุบไม่ได้ และความหมายผิด (AI ไม่ได้เห็นปัจจัยเหล่านั้น)
- แถวดิบของการปลุกจริงเก็บ `RETENTION_DAYS` = 120 วัน (มุมมอง AI เต็มรูปอยู่ใน `AiWakeView` 365 วัน)
- มุมมอง SKIP ใน `AiWakeView` เก็บ 30 วัน (`SKIP_RETENTION_DAYS`) — ใช้แค่ในสถิติ 30 วัน; NOTIFY เก็บ 1 ปี
- ผลลัพธ์: แถวดิบคงที่ ~2 หมื่นแถว + แถวที่ปลุกจริง แทนที่จะโตไม่จำกัด

## 5.5 แก้ Order Block ที่ไม่เคยทำงาน (2026-09-20)

`OB_TOUCH` / `OB_MITIGATED` ไม่เคยยิงเลย และภาพตลาดไม่เคยมีระดับ OB — ไล่เจอ 2 บั๊กซ้อนกันใน `automation/smc`:

1. **หน้าต่างค้นหาไม่ตรงกัน** — `FvgDetection.detect` หา FVG แค่ 30 แท่งท้าย (ค่าเริ่มต้น) แต่ `OrderBlockDetection`
   ค้น swing ทั้งหน้าต่าง (300 แท่ง) และรับ OB เฉพาะที่มี FVG ห่าง ≤ 5 แท่ง → OB ที่เก่ากว่า 30 แท่งหาคู่ไม่เจอ
   วัดบนแท่ง BTC จริง: FVG 3 อัน (ท้ายสุดทั้งหมด) → **OB 0 อัน**
   แก้: `SmcEngine` ส่ง FVG ที่ค้นทั้งหน้าต่างให้ OB (`activeFvgs` สำหรับเข้าเทรดยังเป็น 30 แท่งท้ายเหมือนเดิม) → OB 8–11 อัน/หน้าต่าง
2. **นับ mitigation เร็วเกินไป** — เดิมเริ่มนับ "ราคากลับมาแตะโซน" ตั้งแต่แท่งถัดจาก OB ซึ่งคือ impulse ที่สร้าง OB นั้นเอง
   → OB เกือบทุกอันถูกตีเป็น "ถูกแตะแล้ว" ทันทีที่เกิด (24 หน้าต่างมี OB ที่ยัง active แค่ 1)
   แก้: เริ่มนับหลัง `breakIndex` (แท่งที่ทะลุโครงสร้าง) → **17/24 หน้าต่างมี OB ที่ยัง active**

ผลกระทบ: ระดับ Demand/Supply OB เข้าภาพตลาดของ AI แล้ว, ปัจจัย OB ทำงานได้, และฝั่งระบบสัญญาณได้ setup แบบ OB,
กำแพง OB, คะแนน confluence ของโซน กลับมาทำงานตามที่ออกแบบไว้ (เทสต์ `SmcOrderBlockTest`)
