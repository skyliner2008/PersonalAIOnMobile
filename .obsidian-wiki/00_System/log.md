## 2026-09-24 — ✅ ถาม 4 ข้อ + ขัดจังหวะระหว่างรายงาน ได้ครบ; แก้ทักทายซ้ำในเทิร์นรับงาน
- **User Request**: "รอบที่ 2 พูดขัดจังหวะตอนกำลังรายงานผล ให้วิเคราะห์ SMC เพิ่ม ตอบได้หมด 4 อย่าง แต่ยังมีกล่าวทักทายซ้ำหลายรอบ"
- **logcat 03:44–03:47**: ปฏิทิน → ราคาทอง → BTC 5 มิติ ถามรัว + "SMC ทองคำ M15" แทรกระหว่างรายงาน
  → รายงานครบ 4 ข้อ (ปฏิทิน, ราคาทอง, BTC, SMC) ตามลำดับ ไม่มีส่งแทรก/เสียงซ้อน/tool วนซ้ำ
- **ทักทายซ้ำ**: เกิดเฉพาะ **เทิร์นรับงาน** หลัง `DELEGATED` (45:09 BTC, 46:25 SMC) — ข้อความรับงานบอกแค่ "ตอบรับสั้นๆ 1 ประโยค"
  โมเดลบางครั้งหยิบประโยคทักทายเปิดเซสชัน (ประโยคสั้นที่ persona กำหนดให้พูดคำต่อคำ) มาใช้; ครั้งที่พูด "กำลังดึง…" (44:58) ถูกต้อง
- **แก้**: `LiveProtocol.delegatedAck` กำหนดประโยครับงานชัด `DELEGATED_ACK_SENTENCE` = "รับทราบ กำลังให้ผู้ช่วยดึงข้อมูลให้"
  + "ห้ามทักทาย ห้ามพูดประโยคอื่น"
- ถ้ายังเกิด: ขั้นต่อไปคือย้ายประโยคทักทายคำต่อคำออกจาก persona ไปไว้ในข้อความเปิดเซสชันครั้งเดียว
- **Verify**: เทสต์ 542 ผ่าน ข้าม 5; ติดตั้งบน SM-S908E แล้ว เปิดแอปได้ ไม่มี crash; ยังไม่ได้ทดสอบด้วยเสียง

## 2026-09-24 — ✅ ยืนยันบนเครื่องจริง: ถาม 3 ข้อรัว ได้คำตอบด้วยเสียงครบ (commit 0d31eab)
- **logcat 03:41–03:43** (SM-S908E, gemini-3.1-flash-live-preview):
  - 41:28 "วิเคราะห์ BTC 5 มิติ" → `DELEGATED` ทันที → ลูกน้องเสร็จใน 1.3 วิ → เข้าคิว
  - 41:32 "ปฏิทินตัวเลขเศรษฐกิจสัปดาห์นี้" → `DELEGATED` → เสร็จใน 0.3 วิ → เข้าคิว (2)
  - 41:37 "ราคาทองคำ" → ตอบทันที
  - 41:58 ตอบทองจบ + เงียบ → "ส่วนเรื่องผลวิเคราะห์ BTC 5 มิติที่บอสขอไว้…" (พูดจบ)
  - 42:45 → "ส่วนเรื่องปฏิทินตัวเลขเศรษฐกิจสัปดาห์นี้…" (พูดจบ)
- ไม่มี: ส่งแทรกระหว่างผู้ใช้พูด (ไมค์เงียบ 23 วิ/70 วิ ตอนส่ง), เสียงซ้อน, tool วนซ้ำ, ทักทายซ้ำ, tool ถูกยกเลิก
- ประโยครับงานของข้อที่ถามทับไม่ได้พูด (ผู้ใช้พูดข้อถัดไปทับ) — ตามออกแบบ
- **สถาปัตยกรรมที่ใช้ได้**: หัวหน้า (Live) ส่งต่อ tool วิเคราะห์ให้ลูกน้อง (`DELEGATED`) + คิวรายงาน (`PendingAnswerQueue`)
  ส่งเฉพาะตอนเงียบจริง (server ≥1.5 วิ, ไมค์ ≥2.5 วิ, ไม่มีเสียงเล่นค้าง, ไม่มี tool ค้าง) + ส่งซ้ำเมื่อถูกกลืน + กัน tool วนซ้ำ

## 2026-09-24 — กันโมเดลวนเรียก tool เดิม + รอให้ server ปิดเทิร์นผู้ใช้ก่อนส่งรายงาน
- **จาก logcat 03:32–03:33**:
  - รายงานที่ถูกกลืนถูกส่งใหม่ได้แล้ว (`↩️ รายงาน "วิเคราะห์ BTC 5 มิติ" ถูกกลืน … ส่งใหม่`) และ AI เริ่มพูด "ส่วนเรื่องผลวิเคราะห์ BTC 5 มิติ…"
    ก่อนถูกคำถามราคาทองของผู้ใช้ตัดอีกรอบ
  - **โมเดลเรียก `trading_price` XAUUSD วนซ้ำ 7 ครั้งทุก 0.85 วิ** ไม่พูดคำตอบเลย
  - ส่งรายงานที่ "ไมค์เงียบ 1633ms" แต่ transcript ของผู้ใช้มาถึง 22ms ต่อมา — ผู้ใช้หยุดพูดแล้ว แต่ server ยังรอเงียบ 1.2 วิ
    (`silenceDurationMs`) ก่อนปิดเทิร์น ไมค์ทำงานถูก ช่องรอ 1 วิสั้นกว่าเวลาที่ server ใช้ปิดเทิร์น
- **แก้**:
  - `DuplicateCallGuard` (pure + เทสต์): tool เดิม + args เดิม ในเทิร์นเดียวกัน (20 วิ) → ไม่รันใหม่ ตอบผลเดิมพร้อม `[DUPLICATE CALL]`
    ให้พูดตอบทันที (ยังรันอยู่ → บอกให้รอ); อ้างผลด้วย callId เพราะ args อาจถูกแก้ระหว่างทาง; log `🔁 … ถูกเรียกซ้ำ`
  - คิวรายงานรอไมค์เงียบ ≥ 2.5 วิ (เดิม 1 วิ)
- **Verify**: `DuplicateCallGuardTest` 4 เคส — รวม 542 ผ่าน ข้าม 5; ติดตั้งบน SM-S908E แล้ว เปิดแอปได้ ไม่มี crash; ยังไม่ได้ทดสอบด้วยเสียง

## 2026-09-24 — รายงานของลูกน้องที่ถูกคำถามใหม่กลืน → ส่งใหม่หลังตอบข้อใหม่
- **จาก logcat 03:24–03:25**: หัวหน้า/ลูกน้องทำงาน — `DELEGATED` ตอบทันที, ลูกน้องทำเสร็จทั้ง BTC และปฏิทิน, ผลเข้าคิวครบ
  แต่รายงานทั้งสองหาย: คิวส่งรายงาน BTC ที่ 05.284 ("ไมค์เงียบ 1219ms") ขณะผู้ใช้กำลังพูด "ปฏิทิน…" (transcript มาถึง 05.714)
  → โมเดลตอบคำถามใหม่ รายงานถูกกลืน; ปฏิทินที่ 10.127 เหมือนกัน → ได้ยินแค่ราคาทอง
- **ต้นเหตุ**: ไมค์ในเครื่องจับเสียงพูดหลัง AEC/NoiseSuppressor ไม่ได้ที่เกณฑ์ 0.05 — การรอ "เงียบ" อย่างเดียวไม่พอ
- **แก้**:
  - ยืนยันการรายงาน (`settleReportDelivery`): หลังส่งรายงาน ถ้ามี transcript ของผู้ใช้เข้ามาก่อนจบเทิร์น หรือ AI ไม่ได้พูด
    → รายงานถูกกลืน ใส่กลับหัวคิว (`PendingAnswerQueue.requeueFront`, สูงสุด 3 ครั้ง; มีผลใหม่ของ tool เดียวกันแล้วใช้ของใหม่)
    → ส่งใหม่หลัง AI ตอบข้อใหม่จบ; log `↩️ รายงาน … ถูกกลืน … ส่งใหม่หลังตอบข้อใหม่`
  - เกณฑ์เสียงพูดสำหรับคิว 0.02 (UI ยังใช้ 0.05)
- **ยังเกิด**: ทักทายซ้ำในเทิร์นตอบรับงาน (03:25:02 "สวัสดีค่ะบอส" แล้วถูกขัด)
- **Verify**: เทสต์ +2 เคส รวม 538 ผ่าน ข้าม 5; ติดตั้งบน SM-S908E แล้ว เปิดแอปได้ ไม่มี crash; ยังไม่ได้ทดสอบด้วยเสียง

## 2026-09-24 — Live เป็น "หัวหน้า" จริง: ส่งต่องานวิเคราะห์ให้ลูกน้อง แล้วรายงานผลตอนเงียบ
- **User Request**: "ยังไม่โอเค มีเสียงซ้อน 2 แบบ" (logcat 02:06–02:08) + "flow ที่วางไว้ AI Live เป็นหัวหน้า สั่งงานลูกน้องแล้วรายงานผล
  แต่ flow ปัจจุบัน Live ทำงานเองทั้งหมด" — เปรียบเทียบกับ `b1b013d` แล้วผู้ใช้เลือก "ใช้แบบปัจจุบัน แก้ต่อ"
- **เทียบกับ `b1b013d`**: เส้นทาง Live ถึง `8b0b9b3` เปลี่ยนน้อย (guard เฟส B, quota retry, compression 25K/8K)
  กลไกที่ซ้อนขึ้นมาเป็นของ 09-23/24 ทั้งหมด; ที่ `b1b013d` เอง Live ก็เรียก tool วิเคราะห์ตรงๆ (agent_task_start มีแต่โมเดลไม่ค่อยเลือกใช้)
- **รอบนี้แก้ (commit 367389c)**: คิวรอเสียงในเครื่องเล่นจบ (เสียง AI ที่ค้างในลำโพง + TTS สำรอง — ต้นเหตุเสียงซ้อน),
  ป้ายของผลในคิวอ้างชื่อ tool (เดิมผล BTC ติดป้าย "ปฏิทิน"), ประวัติเก็บคำตอบ AI แค่ 120 ตัวอักษร + ระบุว่าตัวเลขเก่า
  (เดิมโมเดลพูดผลวิเคราะห์ BTC ของ session ก่อนซ้ำแทนการเรียก tool)
- **หัวหน้า/ลูกน้อง (`LiveToolBridge`)**: tool อ่านข้อมูล/วิเคราะห์ (`LiveIntentMatchers.DELEGATED_TOOLS` — deep analysis, ปฏิทิน, SMC,
  sentiment, fundamental, ข่าว, scan …) → ตอบ Live ทันทีด้วย `DELEGATED:` (หัวหน้ารับงาน 1 ประโยค ห้ามเดาผล) → รันเส้นทางเดิมเบื้องหลัง
  (guard/TF/การ์ดแชท) → `respond()` ส่งผลเข้าคิว → รายงานตอนเงียบจริง; guard ของงานที่ส่งต่อก็รายงาน (ไม่ทิ้งเงียบ)
  ไม่ส่งต่อ: `trading_price`, คำสั่ง MT5, backtest, alert, skill chain
  ไม่มีช่วงรอผลในเทิร์น → server ไม่มีอะไรให้ยกเลิก; ถามรัวได้ทุกข้อ
- งานเบื้องหลังแบบ LLM (`agent_task_start`/backtest) รายงานผ่านคิวเดียวกัน (`orchestrator.enqueueLiveReport`) — เดิมส่งทันที พูดทับผู้ใช้ได้
- log: `👷 ส่งต่อ … ให้ผู้ช่วยทำเบื้องหลัง`, `📥 เก็บผล …`, `📤 ตอบคำถามที่ค้าง … (ไมค์เงียบ …ms, server เงียบ …ms)`
- **Verify**: เทสต์ 536 ผ่าน ข้าม 5; ติดตั้งบน SM-S908E แล้ว เปิดแอปได้ ไม่มี crash; ยังไม่ได้ทดสอบด้วยเสียง

## 2026-09-24 — แก้: session ค้างเงียบหลังถามรัว — ห้ามส่งข้อความแทรกระหว่างผู้ใช้พูด
- **User Request**: "ในแชทมีผล tool ทั้ง 3 เข้ามาถูกต้อง แต่ไม่มีเสียงพูดของ AI" (logcat 01:48)
- **ต้นเหตุ (จาก fix ของผมเอง)**: tool ที่ถูกยกเลิกขณะ "ยังไม่มี transcript ใหม่" ถูกตีว่าเป็นเสียงรบกวน แล้วส่ง `[SYSTEM]` realtime text ทันที
  แต่ transcript ของ server มาช้ากว่าเสียงพูดจริง 2–3 วิ — ที่ 24.8 และ 29.8 ผู้ใช้กำลังพูดคำถามถัดไปอยู่
  (transcript มาถึง 27.8 และ 32.3) → ส่งแทรก 2 ครั้ง แล้วหลังผลราคาทอง (35.8) โมเดลเงียบค้างจนปิด session
- **แก้**:
  - ทุกผลที่ค้างผ่าน `PendingAnswerQueue` เท่านั้น — เลิกส่ง realtime text ทันทีจาก `LiveToolBridge`
  - ส่งข้อค้างเฉพาะตอนเงียบจริง (`isConversationIdle`): server ไม่มีกิจกรรม ≥1.5 วิ (transcript/เสียง AI/tool call/interrupted/turn complete),
    **ไมค์ในเครื่องไม่ได้ยินเสียงพูด ≥1 วิ** (`VoiceController` → `noteLiveUserVoice` — มาก่อน transcript 2–3 วิ; ถ้าเสียงรอบข้างดังตลอด
    ยอมเมื่อ server เงียบ ≥6 วิ), ไม่มี tool ค้าง (`inFlightToolCalls`, ค้างเกิน 30 วิถือว่าหลุด); ยังไม่เงียบ → รอต่อได้ถึง 60 วิ
  - ตัวกันค้าง: ส่งผล tool แล้วโมเดลไม่พูดและไม่จบเทิร์นใน 12 วิ → ส่งผลซ้ำผ่านคิว (log `⚠️ โมเดลเงียบ 12 วิหลังผล …`)
- **Verify**: เทสต์ 531 ผ่าน ข้าม 5; ติดตั้งบน SM-S908E แล้ว เปิดแอปได้ ไม่มี crash; ยังไม่ได้ทดสอบด้วยเสียง

## 2026-09-24 — Live: คำตอบที่ถูกตัดหลัง tool ส่งผลแล้ว ก็เข้าคิวตอบทีหลัง
- **User Request**: "tool ทำงานได้หมด แต่ยังตอบด้วยเสียงแค่ข้อล่าสุด" — ทดสอบ 2 รอบสลับลำดับคำถาม (logcat 01:34, 01:36)
- **กรณีที่คิวรอบก่อนไม่ครอบคลุม**: tool ส่งผลถึงโมเดล**สำเร็จ** (`✅ Tool response sent`) แล้วผู้ใช้พูดข้อถัดไปทับก่อน AI เริ่มพูด
  → `Interrupted` → `Turn Complete (audio: 0)` server ทิ้งคำตอบทั้งเทิร์น — ไม่ใช่ "ถูกตัดก่อนเรียก tool" และไม่ใช่ "tool ถูกยกเลิก"
- **แก้**: `LiveGeminiService` จำผล tool ที่ส่งแล้วในเทิร์น (`toolResultsThisTurn`) + AI เปล่งเสียงแล้วหรือยัง (`modelSpokeThisTurn`
  — ตัวนับเสียงถูกรีเซ็ตตอน interrupted จึงใช้ไม่ได้) → `PendingAnswerQueue.onTurnEnded` (pure, มีเทสต์) ตัดสินว่าอะไรเข้าคิว:
  ถูกตัดก่อนเรียก tool → คำถามเข้าคิว; tool ส่งผลแล้วแต่ AI ยังไม่ได้พูด → ผลเข้าคิว; AI พูดไปแล้วบางส่วน → ไม่ตอบซ้ำ
  log: `📥 เทิร์นถูกตัดก่อน AI ได้พูด — เก็บ N ข้อไว้ตอบทีหลัง`
- **ยังเกิด**: ทักทายซ้ำก่อนตอบ BTC (01:36:44) — เกิดหลังเทิร์นก่อนหน้าถูกตัดติดกันหลายครั้ง (ยังไม่แก้)
- **Verify**: `PendingAnswerQueueTest` +3 เคส (ลำดับจริงจาก log) — รวม 531 ผ่าน ข้าม 5; ติดตั้งบน SM-S908E แล้ว เปิดแอปได้ ไม่มี crash

## 2026-09-24 — Live: ถามหลายข้อติดกันได้ (คิวคำถามค้าง) + ข้ามบทวิเคราะห์เสริมใน Live
- **User Request**: "สั่งแบบหลาย tool ไม่ได้ แต่สั่งทีละ tool ได้" (logcat 01:19–01:22)
- **ลำดับจริง (ถามรัว)**: "วิเคราะห์ BTC 5 มิติ" → ถูกตัดก่อนเรียก tool (`user="วิเคราะห์ BTC 5 มิติ" jarvis="-"`)
  → "ปฏิทินเศรษฐกิจ" → tool ถูกยกเลิกเพราะผู้ใช้ถามราคาทองต่อ → fix ก่อนหน้า**ทิ้งผลเมื่อผู้ใช้ถามเรื่องใหม่** → ได้แค่ราคาทอง
  ถามทีละข้อ (session 2) ได้ครบ — Gemini Live ตอบเฉพาะคำถามล่าสุดเสมอ
- **แก้ — `PendingAnswerQueue`** (คิวสูงสุด 3 ข้อ, หมดอายุ 2 นาที):
  - เทิร์นที่ถูกตัดก่อน AI ได้ตอบ/เรียก tool (มีคำถาม, ไม่มีคำตอบ, ไม่มี tool call, interrupted) → เข้าคิว
  - tool ที่ถูกยกเลิกเพราะผู้ใช้ถามเรื่องใหม่ → รันจนเสร็จแล้วเก็บผลเข้าคิว (`LiveToolBridge` → `enqueuePendingToolResult`)
  - ตอบข้อปัจจุบันจบ + เงียบ 1.5 วิ → ส่ง `[PENDING QUESTION] ก่อนหน้านี้ผู้ใช้ถามว่า …` ให้ตอบต่อทีละข้อ (ไม่พูดทับผู้ใช้)
  - ผู้ใช้ถามข้อเดิมซ้ำเอง → ตัดออกจากคิว; พูด "ไม่เอาแล้ว/ยกเลิก/พอแล้ว" → ล้างคิว; คำรับ/อุทาน ("ครับ", "อืม") ไม่เข้าคิว
  - ทิ้งผลของ call ที่ถูกยกเลิกเฉพาะเมื่อโมเดลเรียก tool เดิม **ด้วย args เดิม** ซ้ำเอง (เดิมเทียบแค่ชื่อ tool)
  - log: `📥 คำถามถูกตัดก่อนได้ตอบ…`, `📥 เก็บผล … ไว้ตอบทีหลัง`, `📤 ตอบคำถามที่ค้าง…`
- **ข้ามบทวิเคราะห์เสริมใน Live**: tool ที่ Live เรียก (ติด `LiveToolCallContext`) ไม่รอโมเดลตัวที่สอง — โมเดล Live วิเคราะห์ข้อมูลดิบเอง
  (ปฏิทินเคยใช้ 8.4 วิ เพราะรอ 503 → fallback; ยิ่งรอนานยิ่งโดนเสียงแทรกยกเลิก) — ยกเว้นตัวจับคู่ sector (`skipInLive = false`)
- **ยังเกิด**: AI ทักทายซ้ำก่อนตอบปฏิทิน 1 ครั้ง (`⚠️ ทักทายซ้ำกลางบทสนทนา`) — ตอนนั้น tool ใช้ 8.4 วิ; หลังตัดการรอโมเดลที่สองต้องดูว่ายังเกิดไหม
- **Verify**: `PendingAnswerQueueTest` 6 เคส — รวม 528 ผ่าน ข้าม 5; ติดตั้งบน SM-S908E แล้ว เปิดแอปได้ ไม่มี crash; ยังไม่ได้ทดสอบด้วยเสียง

## 2026-09-24 — Live ตัดจบคำพูดเร็วเกิน (ต้นเหตุ tool ถูกยกเลิก) + ประวัติแชทพิมพ์ขาดบทสนทนาเสียง
- **User Request**: log 01:04 + "ตามหลักไม่ควรยกเลิก เพราะมีระบบ multi session" + "ก่อนหน้านี้แชทแสดงเสียงผู้ใช้และ AI หลังเอาออกมันแปลกๆ"
- **ผลจาก fix ก่อนหน้า**: tool ที่ถูกยกเลิกส่งผลกลับได้แล้ว (`⬆ Sent realtime text: [SYSTEM] คำถามของผู้ใช้ "วิเคราะห์ BTC 5 มิติ"…`)
- **ต้นเหตุจริงของการยกเลิก — ไม่ใช่เสียงรอบข้าง**: transcript คำถามถัดไปขึ้นต้นด้วย "ล่ะ ครับ" = ท้ายประโยคก่อนหน้า
  → server ตัดสินว่าผู้ใช้พูดจบตั้งแต่เว้นจังหวะกลางประโยค เรียก tool ทันที พอพูดคำท้ายต่อจึงนับเป็นพูดแทรกแล้วยกเลิก tool
  ค่า `end_of_speech_sensitivity` ไม่เคยถูกตั้ง → ค่าเริ่มต้น `END_SENSITIVITY_HIGH` ("ends speech more often" ตามเอกสาร)
  → ตั้ง `END_SENSITIVITY_LOW` (ย้าย config เป็น `LiveRealtimeInputConfig.default()` + เทสต์)
- **NON_BLOCKING (async function calling)** ซึ่งเป็นวิธีของ Gemini สำหรับงานเบื้องหลัง — เอกสารระบุว่า *ยังไม่รองรับ* ใน gemini-3.1-flash-live
  จึงใช้วิธีส่งผลของ call ที่ถูกยกเลิกเป็น realtime text (ช่องทางเดียวกับ multi-session agent) แทน
- **สิ่งที่พังตามการซ่อนคำพูดในแชท (`b1b013d`)**: มี 2 จุดที่อ่าน "ประวัติ" จากรายการแชท
  1. ประวัติตอนเปิด Live — แก้แล้วรอบก่อน (อ่านจากฐานข้อมูล)
  2. `ChatController.buildHistorySnapshot` (แชทแบบพิมพ์) — คุยด้วยเสียงแล้วมาพิมพ์ต่อ AI ไม่รู้ว่าเพิ่งคุยอะไร → อ่านจากฐานข้อมูลแล้ว
  ทั้งสองใช้ `recentConversationTurns` ที่ตัดคำทักทายเปิดเซสชันและผล tool ดิบในจุดเดียว
- **Verify**: เทสต์ 522 ผ่าน ข้าม 5; ติดตั้งบน SM-S908E แล้ว เปิดแอปได้ ไม่มี crash; ยังไม่ได้ทดสอบด้วยเสียง
