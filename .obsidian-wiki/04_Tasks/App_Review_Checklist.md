...
### 1. Provider / ผู้ให้บริการ LLM ✅ (2026-08-08 — revamp ใหญ่ เทสเครื่องจริงผ่านแล้ว)

* **ขอบเขต**: `data/providers/*` (Gemini, OpenRouter, OpenAI, Claude, Minimax, Vertex, LiteLlm, ADK), `ModelConfig.kt`, Settings provider UX

* **สิ่งที่ทำแล้ว (2026-07-30)**: ตรวจรายชื่อโมเดลล่าสุดของแต่ละ provider, การเลือก/สลับ/คัดกรองโมเดล (live / chat / free model OpenRouter), ปรับ UX การตั้งค่า provider ให้ถูกต้องและใช้งานสะดวก

* **Revamp 2026-08-08**: เหลือ provider: Gemini / OpenRouter / Groq / MiniMax (ตัด OpenAI, Claude, Vertex, LiteLlm, ADK — ไม่มี free; ตัด NVIDIA NIM ภายหลัง — โมเดลช้า/404 เยอะ), Gemini multi API key (add/edit/del ผ่าน UI) + fallback chain ตั้งเองได้, cross-provider fallback (groq → openrouter → minimax) เลือกเฉพาะโมเดลที่ auto-test ผ่าน (persist `model_caps_v1`), auto-test โมเดล Groq/OpenRouter ผ่านปุ่ม ✓ ใน Settings, 429 backoff ตามเวลาที่ API บอก, degraded retry (ตัด tools/history), กัน tool-call loop, persist toggle Show free only, เพิ่ม timeout 60s → 120s กัน reasoning models ขาดกลางประโยค

* **ผลเทสเครื่องจริง**: Groq ✅ qwen3.6-27b / llama-3.3-70b (8b ติด TPM limit ของ Groq) — OpenRouter ✅ 6 ตัว (nemotron-3-super-120b, ultra-550b, nano-omni-30b, gemma-4-26b, laguna-s-2.1, ling-3.0-tiny) เรียก trading_price ได้ราคาถูกทุกตัว

* **บันทึก**: ดู \[\[log]] entry วันที่ 2026-07-29/30 (Provider + UX Settings) และ 2026-08-08 (รอบ 4-14)

### 2. System Prompt / ตัวตน AI ✅ (2026-07-30)

* **ขอบเขต**: `ai/JarvisPersona.kt`, system prompt ทุกจุดที่ประกอบ, CORE\_IDENTITY

* **สิ่งที่ทำแล้ว**: แก้ AI สับสนตัวตน/คำพูดไม่นิ่ง + สร้าง **CORE\_IDENTITY Setting แบบปรับแต่งได้** (Agent Identity: Name/Creature/Vibe/Gender + User Identity: Name/What to call/Notes) — user แก้เองผ่านเมนู input ได้ และ AI แก้ให้ได้เมื่อ user สั่ง

* **บันทึก**: ดู \[\[log]] entry วันที่ 2026-07-30 (CORE\_IDENTITY)
...
* **รอทดสอบเครื่องจริง**: local ONNX ภาษาไทย, recall\_memory แสดง graph context, auto sleep cycle, migration v3 บน install เก่า
...
### 5. Tool System / การสร้าง Tool ✅ (2026-08-08)
- **สิ่งที่ทำแล้ว**: แก้ AI สร้าง tool ข้าม session ไม่ได้ (`loadCustomTools()` ไม่มี caller → เรียกตอน app start), @Volatile custom maps, JSON injection ปลอดภัยอยู่แล้ว, audit การลงทะเบียน + เพิ่ม declaration `trading_mt5_trade_journal` ที่หาย (declared 45 / active 46 / executor ครบ)
- **เทสเครื่องจริงผ่านแล้ว (2026-08-08)**: custom_gold_check โหลดข้าม session ได้, CRUD ผ่าน AI (สร้าง/ดู/แก้/ลบ) ครบ, deep suite fallback TV + circuit breaker ทำงานถูก, isMt5Prompt false positive แก้แล้ว
- **บันทึก**: ดู [[log]] entry 2026-08-08 (หมวด 5)
- **ประเด็นเดิมที่ตรวจแล้ว**:
...
* **ความคืบหน้าที่เกี่ยวข้อง (2026-08-03)**: เพิ่มหมวด tool ใหม่ "📚 Strategy Library" (3 tools: strategy\_list/search/explain) — คลังกลยุทธ์ Quantpedia 60 แบบในเครื่อง ดู \[\[log]] entry 2026-08-03

### 6. Orchestrator / Intent Routing ✅ (2026-08-08 — เทสเครื่องจริงผ่านแล้ว)

* **แก้+ยืนยันจาก logcat จริง**: root cause โมเดล external ใช้ trading tools ไม่ได้ = `take(12)` ตัด trading tools ทิ้ง (builtin มาก่อน) → sort trading tools ขึ้นหัว list เมื่อ trading context + ซ่อน search_web + ตัด MT5 tools เมื่อไม่ pair + system prompt บอกชัดว่า TV tools ไม่ต้อง MT5 — ผล: Groq/OpenRouter ทุกตัวเรียก `trading_price` ได้ราคาถูก
* **ยังเปิดไว้**: JarvisPlanner dead code, IntentClassifier tie-break, รวม TradingIntentUtility/IntentClassifier (ประเด็นเดิมด้านล่าง ไม่กระทบการใช้งาน)
...
* **สิ่งที่ทำแล้ว**: แก้เสียงตอบไม่หมด/เงียบ 5 จุด — F1 คืน transcription config ใน setup, F2 handle `interrupted` → flush playback, F3 audio flow มี buffer (แยก network/playback), F4 แก้ bridge instruction ขัด LIVE\_RULES, F5 TTS fallback เมื่อ turn ไม่มีเสียง + `stopPlaying()` resume หลัง flush

* **ทดสอบเครื่องจริงแล้ว (2026-08-03)**: เสียงตอบครบทุก turn ยืนยันจาก log round5 + news\_check2/3 (audio 1.5-1.7MB/turn) + ปรับ VOICE RULE เป็น 5-8 ประโยคมีสาระ (แก้ตอบสั้นเกิน) — ยังไม่ได้เทส barge-in ทับเสียงเก่าโดยตรง

* **บันทึก**: ดู \[\[log]] entry "Live Voice Fix รอบ 2 (หมวด 7)"

* **เสริม Voice Profile ↔ Identity (2026-08-09 เทสเครื่องจริงผ่านแล้ว)**:
  * เปลี่ยนเสียงผ่าน AI/Settings → ผูก gender/vibe + คำลงท้าย (ครับ/ค่ะ) persist ลง Core Memory (จำข้าม session)
  * Settings > Identity เพิ่ม dropdown Voice Profile 30 เสียง (♀/♂ + tone)
  * แก้ AI เงียบหลังเปลี่ยนเสียง: greeting-on-ready ผูกกับ setupComplete + ส่งผ่าน `realtimeInput.text` (พิสูจน์แล้วว่า clientContent ไม่ trigger generation ขณะ audio streaming)
  * แก้ AI bluff เปลี่ยนเสียงโดยไม่เรียก tool: LIVE_RULES ข้อ 7 + tool description บังคับเรียก voice_set_profile, โจทย์กว้างต้องเสนอเสียง+รอยืนยันก่อน
  * ลบระบบ resume-task เดิมซ้ำหลังเปลี่ยนเสียง (เปลี่ยนแล้วพูดยืนยัน 1 ประโยคอย่างเดียว)
  * ซ่อน transcription ฝั่ง user ของ live voice ตอนโหลดประวัติแชท (ยังเก็บใน DB ให้ AI จำ แต่ไม่แสดง)
  * แก้ user identity ถูกรีเซ็ต: applyVoiceIdentity persist เฉพาะ agent_gender/agent_vibe

### 8. Scheduled Tasks / Alert Automation ✅ (2026-08-08 เทสเครื่องจริงผ่านแล้ว)
...
* **ประเด็นที่ต้องดู**:
  ...
  * [ ] ทบทวน \[\[06\_Virtual\_Price\_Alert\_Roadmap]] ว่าทำไปถึงไหน
...
### 11. Camera / Vision ✅ (2026-08-08 — เทสเครื่องจริงผ่านแล้ว)

* **เทสผ่านโดย user**: Live vision อธิบายสิ่งที่เห็นได้ถูกต้อง (โลชั่นนีเวีย)
* **แก้เพิ่ม (รอยืนยันรอบหน้า)**: อาการ "เปิดตาแล้วเงียบ ต้องถามรอบ 2" — root cause: Live API ไม่เริ่ม turn ใหม่จาก video stream เอง → LiveToolBridge ส่ง auto-prompt กระตุ้นให้สรุปภาพหลัง turn แรกจบ (turnCompleteFlow + sendClientText)
...
* **ประเด็นที่ต้องดู**:
  ...
  * [ ] **Rich Chat Rendering (roadmap จาก user 2026-08-09)**: แสดงตาราง (markdown table), รูปภาพ, กราฟ, infographic ในแชท bubble — ต่อยอดจาก file_write .xlsx ให้ AI สร้างรายงานแล้วแสดงผลสวยในแชทได้โดยไม่ต้องเปิดไฟล์

### 13. Database / Storage ✅ (2026-08-08 — เทสเครื่องจริงผ่านแล้ว)

* **เทสผ่านโดย user**: settings persist ข้าม session, ประวัติแชทอยู่ครบหลังฆ่าแอป, memory จำข้อมูลข้าม session ได้ (recall_memory), alert/custom tools persist (ผ่านจากหมวด 8/5)

* **ขอบเขต**: `commonMain/sqldelight/*` (JarvisDatabase.sq + migrations), `data/GeminiService.kt` ส่วน persistence

* **ประเด็นที่ต้องดู**:

  * [ ] dead queries ที่เหลือ (`updateArchivalAccess`, `getArchivalRecent` — access\_count ไม่เคยถูกใช้)

  * [ ] migration path รวม v1→v2→v3 ทดสอบบน install เก่า

### 14. mt5-core-server / Analytics Scripts 🔶 แก้ไข/ตรวจแล้ว รอทดสอบเครื่องจริง (2026-08-08)
...
### 15. Diagnostic / Logging ✅ (2026-08-08 — ยืนยันจาก logcat จริง)

* **สิ่งที่ทำแล้ว**: log `[Chat] Response complete` แสดง preview คำตอบ 800 ตัวอักษร, log preview ผล tool 300 ตัวอักษร (ใช้จับเคสโมเดลหลอนราคาได้ทันที), ตัด log SSE chunk ดิบของ OpenRouter (logcat เคยพุ่ง 462KB/คำถาม), API key ใน logcat mask หัว-ท้าย (user สั่งให้โชว์แบบ mask ใน logcat เท่านั้น ไม่หลุดเข้า log ใน app)

* **ขอบเขต**: `diagnostic/DiagnosticManager.kt`, logging ทั้งแอป

* **ประเด็นที่ต้องดู**:

  * [ ] log เพียงพอต่อการ debug ปัญหาบนเครื่องจริง (เช่นเคส FRED INTERNAL\_ERROR)
...
| หมวด                        | สถานะ                            | วันที่     |
| --------------------------- | -------------------------------- | ---------- |
| 1. Provider                 | ✅ เสร็จ (revamp + เทสเครื่องจริงผ่าน) | 2026-08-08 |
...
| 5. Tool System / สร้าง Tool | ✅ เสร็จ (เทสเครื่องจริงผ่านแล้ว) | 2026-08-08 |
...
**Links**: \[\[log]] | \[\[Current\_Tasks]] | \[\[ai\_subsystem\_review\_2026-07-29]] | \[\[catalogue]] | \[\[schema]]
